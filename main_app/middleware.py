...
class LoginCheckMiddleWare(MiddlewareMixin):
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        if user.is_authenticated:
            ...
            # Allow access to login, registration, password reset, and authentication views
            if (request.path == reverse('login_page') or 
                request.path == reverse('register_page') or 
                request.path == reverse('do_register') or
                modulename == 'django.contrib.auth.views' or 
                request.path == reverse('user_login') or
                request.path.startswith('/accounts/')): # Allow password reset URLs
                pass
            else:
                return redirect(reverse('login_page'))
